...
3. *(Optional)* Connect robot to your LAN: set `NETWORK_SSID` / `NETWORK_PASS` in [include/core/config.h](include/core/config.h) → access at `http://sesame-robot.local`
...
```
                    ┌─ CmdQueue (FreeRTOS, depth 4) ─────────────┐
                    │                                             │
  TaskWeb   p1 8KB ─┤  Web::pump()                               │
                    │  HTTP handlers → CmdQueue::push()          │
                    │  DNS captive portal                         │
                    │                                             ▼
                    │                               TaskMotor  p2 8KB
                    │                               CmdQueue::pop()
                    │                               → gait / pose dispatcher
                    │                               → Motors::setAngle()
                    │                               → Display::set()
                    │                               Serial CLI
                    │
  TaskDisplay p1 4KB┘  Display::tickFace()   → OLED frame advance
                        Display::tickIdle()  → blink state machine
                        Display::tickMarquee() → WiFi info scroll
```
